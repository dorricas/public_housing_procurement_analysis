...
```
```{r echo=FALSE}
custom_colors <- c("2017" = "#609791", "2018" = "#81B4AF", "2019" = "#A3D2CD", "2020" = "#C4BCAB", "2021" = "#CB485C", "2022" = "#CB0511", "2023" = "#B4010B", "2024" = "#740410")
...
ggplot(data = Purcheues_summary, aes(x = city_lms_name, y = total_apartments, fill = factor(year))) +
  ...
  labs(x = "City", y = "Total Apartments Bought",
       title = "Total Apartments Bought by City",
       fill = "Year")

```
```{r}
Purcheuses_group_year <- Purcheuses %>% 
  group_by(year) %>%
  summarize(total_apartments = sum(x3_rooms_apartments, na.rm = TRUE) + sum(x4_rooms_apartments, na.rm = TRUE))
Purcheuses_group_year

ggplot(Purcheuses_group_year, aes(x = year, y = total_apartments)) +
  geom_bar(stat = "identity", fill = "#609791") +
  labs(title = "Total Number of Apartments per Year", x = "Year", y = "Total Apartments") +
  theme_minimal()
```
```{r}
Purcheuses
