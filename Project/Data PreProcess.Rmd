...
```{r}
Kneset20 <- read.csv("../data/Translated/kneset20.csv")
...
```{r}
Kneset20 <- Kneset20[-1,]
Kneset20 <- Kneset20 %>% select(-X_id)
Kneset20
```


```{r}
Kneset20_dup <- Kneset20
citynames_and_id <- Kneset20 %>% distinct(settlement.ID,settlement.name)
cols_to_sum <- names(Kneset20)[7:ncol(Kneset21)]


Kneset20_chopped <- Kneset20_dup[, 7:ncol(Kneset20_dup)]
...
new_elections_sum$VoterPrecentage <- new_elections_sum$Voters/new_elections_sum$BZB
new_elections_sum$Party.Precentage <- new_elections_sum$max_value/new_elections_sum$Voters
Kneset20_summed <- new_elections_sum
...
```{r}
# write.csv(Kneset21_summed, "../data/Translated/Kneset20_summed.csv")
...
```{r}
SocioEco <- read.csv("../data/Translated/SocioEconomic 2017 2019.csv")
LamasNames <- read.csv("../data/Translated/LamasNamesCodes.csv")
```

```{r, renaming LamasNames to currected translation}
LamasNames <- LamasNames %>% 
  mutate(
    lamas.name = case_when(
    lamas.name =="horizons" ~ "Ofakim",
    lamas.name =="The view of the Galilee" ~ "Nof Hagalil",
    lamas.name == "my peace" ~  "Shlomi" ,
    lamas.name == "routes" ~ "Netivot",
    lamas.name == "horizons" ~ "Ofakim",
    lamas.name == "The view of the Galilee" ~ "Nof Hagalil",
    lamas.name == "my peace" ~  "Shlomi" ,
    lamas.name == "routes" ~ "Netivot",
    lamas.name == "streets" ~ "Rehovot",
    lamas.name == "Area" ~ "Azur",
    lamas.name == "רחובות" ~ "Rehovot",
    lamas.name == "fresh" ~ "Raanana",
    lamas.name == "sons of Aish" ~ "Bnei Aish",
    lamas.name == "a horse" ~ "Susya",
    lamas.name == "scythe" ~ "Hermesh",
    lamas.name == "pestle" ~ "Eli",
    lamas.name == "ridges" ~ "Rekhasim",
    lamas.name == "crimson" ~ "Argaman",
    lamas.name == "earing" ~ "Harish",
    lamas.name == "anemone" ~ "Kalanit",
    lamas.name == "I will tell" ~ "Metzad",
    lamas.name == "crop" ~ "Yevul",
    lamas.name == "stake" ~ "Yated",
    lamas.name == "Come on, Zoran" ~ "Kadima-Zoran",
    lamas.name == "a fence" ~ "Gedera",
    lamas.name == "measles" ~ "Hatzeva",
    lamas.name == "Furia-Neva is working" ~ "Poria–Neve Oved",
    lamas.name == "Ptolemy Yosef" ~ "Talmei Yosef",
    lamas.name == "Solid stones" ~ "Avnei Eitan",
    lamas.name == "horn" ~ "Tzofar",
    lamas.name == "Horns of Samaria" ~ "Karnei Shomron",
    lamas.name == "sapphire" ~ "Sapir",
    lamas.name == "dawn" ~ "Tzohar",
    lamas.name == "truffle" ~ "Kmehin",
    lamas.name == "Chinese radish" ~ "Kadesh Barnea",
    lamas.name == "towers" ~ "Migdalim",
    lamas.name == "harvesting" ~ "Katzir",
    lamas.name == "emerald" ~ "Bareket",
    lamas.name == "A souvenir of her house" ~ "Mazkeret Batya",
    lamas.name == "darling" ~ "Yakir",
    lamas.name == "tower" ~ "Migdal",
    lamas.name == "Ancients" ~ "Kedumim",
    lamas.name == "face friend" ~ "Ma'ale Hever",
    lamas.name == "on" ~ "Sha'al",
    lamas.name == "Gold leaves" ~ "Alei Zahav",
    lamas.name == "better" ~ "Yitav",
    lamas.name == "Bethel" ~ "Beit El",
    lamas.name == "Sheila" ~ "Shilo",
    lamas.name == "Gems" ~ "Avnei Hefetz",
    lamas.name == "Tarshiha" ~ "Ma'alot-Tarshiha",
    lamas.name == "diligent" ~ "Harutzim",
    lamas.name == "Roy" ~ "Ro'i",
    lamas.name == "flint" ~ "Halamish",
    lamas.name == "roots" ~ "Shorashim",
    lamas.name == "Absalom" ~ "Avshalom",
    lamas.name == "Dotan's introduction" ~ "Mevo Dotan",
    lamas.name == "The Galilee" ~ "Gornot HaGalil",
    lamas.name == "Shafram" ~ "Shfar'am",
    lamas.name == "torches" ~ "Lapidot",
    lamas.name == "Apple village" ~ "Kfar Tapuach",
    lamas.name == "its openings" ~ "Petahya",
    lamas.name == "grape" ~ "Einav",
    lamas.name == "Dubb" ~ "Dovev",
    lamas.name == "antimony" ~ "Kahal",
    lamas.name == "Yavnal" ~ "Yavne'el",
    lamas.name == "hardware" ~ "Hamra",
    lamas.name == "garden fruit" ~ "Pri Gan",
    lamas.name == "Almond" ~ "Shaked",
    lamas.name == "Head turned" ~ "Rosh Pinna",
    lamas.name == "Frankincense" ~ "Ma'ale Levona",
    lamas.name == "Abraham's demons" ~ "Sdei Avraham",
    TRUE ~ lamas.name  # Keeps other values unchanged
    )
  )
```


```{r, joining currected translation to SocioEco DF}
SocioEco <- left_join(SocioEco, LamasNames, by = c("CODE.OF.LOCALITY" = "lamas.code"))
SocioEco <- SocioEco[,c(1,6,3,4,5)]
SocioEco
```

```{r}
Kneset21_summed_joined <- left_join(Kneset21_summed,SocioEco, by = c("settlement.name" = "lamas.name" ))
Kneset21_summed_joined
# # Create the bar plot
# ggplot(Kneset21_summed_joined, aes(x = CLUSTER.2019)) +
#   geom_bar(fill = "blue", color = "black", alpha = 0.7) +
#   facet_wrap(~ max_col_name, scales = "free_y") +
#   labs(title = "Bar Plot of INDEX.POPULATION.2019 by max_col_name",
#        x = "INDEX.POPULATION.2019",
#        y = "Count") +
#   theme_minimal()


# Summarize and filter the data
filtered_data <- Kneset21_summed_joined %>%
  group_by(max_col_name, CLUSTER.2019) %>%
  summarise(count = n()) %>%
  ungroup() %>%
...
# Create the bar plot
ggplot(filtered_data, aes(x = CLUSTER.2019, y = count)) +
...
```{r}
Properties_rooms <- left_join(Properties_rooms,LamasNames, by = c("city_lms_name" = "lamas.name"))
```
...
Properties_room_sum <-left_join(Properties_room_sum,Kneset25_summed, by = c("city_lms_name" = "settlement.name"))
Properties_room_sum <- Properties_room_sum %>% drop_na()
Properties_room_sum <- Properties_room_sum[order(Properties_room_sum$VoterPrecentage),]

total_max <- max(Properties_room_sum$total, na.rm = TRUE)
Properties_room_sum$total.precentage <- (Properties_room_sum$total / total_max) * 100
```

```{r}
ggplot(data = Properties_room_sum, aes(x= VoterPrecentage,y  = total)) + geom_point()
```
...
Joined_Purch_Kneset20 <- Purcheuses %>%
  filter(year < 2019) %>%
  left_join(Kneset20_summed, by = c("city_lms_name" = "settlement.name"))

Joined_Purch_Kneset21 <- Purcheuses %>%
  filter(year == 2019) %>%
  left_join(Kneset21_summed, by = c("city_lms_name" = "settlement.name"))

Joined_Purch_Kneset22 <- Purcheuses %>%
  filter(year == 2020) %>%
  left_join(Kneset22_summed, by = c("city_lms_name" = "settlement.name"))

Joined_Purch_Kneset23 <- Purcheuses %>%
  filter(year == 2021) %>%
  left_join(Kneset23_summed, by = c("city_lms_name" = "settlement.name"))

Joined_Purch_Kneset24 <- Purcheuses %>%
  filter(year == 2022) %>%
  left_join(Kneset24_summed, by = c("city_lms_name" = "settlement.name"))

Joined_Purch_Kneset25 <- Purcheuses %>%
  filter(year > 2022) %>%
  left_join(Kneset25_summed, by = c("city_lms_name" = "settlement.name"))
...
Purchuses_Elections <- Purchuses_Elections %>% 
  mutate(
    max_col_name = case_when(
      ...
      max_col_name == "Hadash" ~ "Hadash_Raam",
      max_col_name == "Kahul_lavan" ~ "Kahul_Lavan",
      TRUE ~ max_col_name  # keep unchanged if no match
    )
  )

Party_names <- unique(Purchuses_Elections$max_col_name)
Coalition <- c("Halicud","Yahadut_Hatora")

# Create a new coalition column with initial value 0
coalition <- integer(nrow(Purchuses_Elections))

# Bind the new column to Purchuses_Elections
Purchuses_Elections <- bind_cols(Purchuses_Elections, coalition = coalition)

# Mutate to increment coalition by 1 if max_col_name is in Coalition
Purchuses_Elections <- Purchuses_Elections %>% 
  mutate(
    coalition = ifelse(max_col_name %in% Coalition, coalition + 1, coalition)
  )

coalition_subset <- Purchuses_Elections %>% 
  filter(coalition == 1)

opposition_subset <- Purchuses_Elections %>% 
  filter(coalition == 0)

coalition_subset_summary <- coalition_subset %>%
  group_by(city_lms_name, year) %>%
  summarise(total_apartments = sum(x3_rooms_apartments + x4_rooms_apartments)) %>% 
  drop_na() %>% 
    group_by(year) %>%  summarise(total = sum(total_apartments))

opposition_subset_summary <- opposition_subset %>%
  group_by(city_lms_name, year) %>%
  summarise(total_apartments = sum(x3_rooms_apartments + x4_rooms_apartments)) %>% 
  drop_na() %>% 
  group_by(year) %>%  summarise(total = sum(total_apartments))
...
ggplot(data = opposition_subset_summary, aes(x = year, y = total)) + 
  geom_bar(stat = "identity")

ggplot(data = coalition_subset_summary, aes(x = year, y = total)) + 
  geom_bar(stat = "identity")
